...
if [ -z "$PRIBIT_CONNECT_ID" ]; then
  # .env 파일에서 기존 값 확인
  existing_id=$(get_env_value "PRIBIT_CONNECT_ID")
  if [ -n "$existing_id" ]; then
    PRIBIT_CONNECT_ID="$existing_id"
...
if [ -z "$PRIBIT_CONNECT_PASSWORD" ]; then
  existing_password=$(get_env_value "PRIBIT_CONNECT_PASSWORD")
  if [ -n "$existing_password" ]; then
    PRIBIT_CONNECT_PASSWORD="$existing_password"
...
echo ""
echo "=== AutoA AppleScript 환경 설정 완료 ==="
echo "모든 설정이 완료되었습니다."
