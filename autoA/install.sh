...
# Google Drive 파일 ID 추출 (기본 버전)
extract_file_id_basic() {
    local url="$1"
    local file_id
...
download_with_curl_basic() {
    ...
    # 파일 ID 추출
    local file_id
    file_id=$(extract_file_id_basic "$url")
    if [[ $? -ne 0 ]]; then
        return 1
    fi
    ...
    if curl -c "$cookies_file" -L -o "$temp_file" "$download_url"; then
        ...
        if [[ -n "$confirm_token" ]]; then
            ...
            # 확인 토큰을 사용하여 실제 다운로드
            local final_url="https://drive.google.com/uc?export=download&confirm=${confirm_token}&id=$file_id"
            
            if curl -b "$cookies_file" -L -o "$output_file" "$final_url"; then
...
# 서버 이미지 수동 다운로드 가이드
manual_download_guide_server() {
    log_warning "자동 다운로드에 실패했습니다. 수동 다운로드를 진행해주세요."
    echo ""
    echo "📋 서버 이미지 수동 다운로드 방법:"
    echo "1. 다음 URL로 이동: https://drive.google.com/file/d/1PhD7xtKZo5CmOkcIARM7V2BkZzeIr4H3/view?usp=sharing"
    echo "2. '다운로드' 버튼을 클릭하세요"
    echo "3. 파일을 'server_image.tar.gz'로 저장하세요"
    ...
    if command -v open &> /dev/null; then
        ...
        open "https://drive.google.com/file/d/1PhD7xtKZo5CmOkcIARM7V2BkZzeIr4H3/view?usp=sharing"
    fi
}

# 에이전트 패키지 수동 다운로드 가이드
manual_download_guide_agent() {
    log_warning "자동 다운로드에 실패했습니다. 수동 다운로드를 진행해주세요."
    echo ""
    echo "📋 에이전트 패키지 수동 다운로드 방법:"
    echo "1. 다음 URL로 이동: https://drive.google.com/file/d/1cfMLZE5tto4IHQd4VmIfv0a2nR02MTwW/view?usp=sharing"
    echo "2. '다운로드' 버튼을 클릭하세요"
    echo "3. 파일을 'agent.pkg'로 저장하세요"
    echo "4. 다운로드 완료 후 스크립트를 다시 실행하세요"
    echo ""
    echo "💡 팁:"
    echo "   - 파일이 크면 Google Drive에서 '다운로드할 수 없습니다' 메시지가 나타날 수 있습니다"
    ...
    if command -v open &> /dev/null; then
        log_info "브라우저에서 다운로드 페이지를 엽니다..."
        open "https://drive.google.com/file/d/1cfMLZE5tto4IHQd4VmIfv0a2nR02MTwW/view?usp=sharing"
    fi
}
...
download_server_image() {
    ...
    # 다운로드 헬퍼 스크립트 사용
    if [[ -f "$SCRIPT_DIR/download-helper.sh" ]]; then
        log_info "개선된 다운로드 헬퍼 스크립트를 사용합니다..."
        log_info "이 스크립트는 Google Drive의 직접 액세스 제한을 우회합니다..."
        
        if "$SCRIPT_DIR/download-helper.sh" "$SERVER_FILE_URL" "server_image.tar.gz"; then
        ...
        log_warning "다운로드 헬퍼 스크립트를 찾을 수 없습니다: $SCRIPT_DIR/download-helper.sh"
        log_info "기본 다운로드 방법을 사용합니다..."
        
        # 기본 curl 다운로드 시도
        if download_with_curl_basic "$SERVER_FILE_URL" "server_image.tar.gz"; then
            log_success "서버 이미지 다운로드 완료 (기본 방법)"
            ...
            if [[ -f "server_image.tar.gz" && -s "server_image.tar.gz" ]]; then
                ...
                if tar -tzf server_image.tar.gz > /dev/null 2>&1; then
                    ...
                    log_error "다운로드된 파일이 유효한 tar.gz 형식이 아닙니다"
                    log_info "파일이 손상되었을 수 있습니다. 수동 다운로드를 진행해주세요."
                    manual_download_guide_server
                    exit 1
                fi
            else
...
download_agent_package() {
    ...
    if [[ -f "$SCRIPT_DIR/download-helper.sh" ]]; then
        ...
        if "$SCRIPT_DIR/download-helper.sh" "$AGENT_FILE_URL" "agent.pkg"; then
        ...
        # 기본 curl 다운로드 시도
        if download_with_curl_basic "$AGENT_FILE_URL" "agent.pkg"; then
            log_success "에이전트 패키지 다운로드 완료 (기본 방법)"
